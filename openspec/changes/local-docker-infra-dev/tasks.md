...
- [ ] 4.4 Запустить сценарий из локального UI: сообщение уходит в локальную очередь, обрабатывается локальным воркером, статус доходит до терминального (не виснет в «Выполняем сценарий…»)
  - ЧАСТИЧНО. Проверен round-trip через очередь: сообщение, отправленное в локальный SQS, подобрано локальным воркером (`Message <id> processed` с тем же MessageId) — то есть перехвата задеплоенным бэкендом нет. Запуск именно из UI не выполнен: свежая локальная БД пуста, приложений и сценариев в ней нет, а сид данных вынесен за рамки этой задачи (см. Open Questions в design.md). Закрывается вместе с задачей о сиде
