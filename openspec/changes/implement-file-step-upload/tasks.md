...
- [x] 5.3 E2e против реального `http://10.46.142.217/llm-doc-recognizer`: сценарий «Распознать документ» (шаг сконвертирован в `file` прямо в БД — в билдере пока нет конфигуратора файлового шага), single-загрузка PDF → multipart в поле `doc` + `data`-константа → опрос `/task/{task_id}` до `completed` → результат шага = распознанный текст
